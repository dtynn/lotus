...
func NewMergeClient(addr string, namespace string, outs []interface{}, header http.Header) (ClientCloser, error) {
	...
	rcli, closer := newRPCClient(namespace, addr, header, http.DefaultTransport)
...
func newRPCClient(namespace, remote string, header http.Header, rt http.RoundTripper) (*rpcClient, ClientCloser) {
